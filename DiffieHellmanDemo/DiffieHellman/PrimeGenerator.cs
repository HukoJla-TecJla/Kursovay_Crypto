...
internal static class PrimeGenerator
{
    ...
    public static BigInteger GenerateSafePrime(int bitLength)
    {
        ...
        BigInteger q = GeneratePrime(bitLength - 1);
        BigInteger p = 2 * q + 1;
        
        int attempts = 0;
        const int maxAttempts = 100;
        
        while (!IsProbablyPrime(p, 20) && attempts < maxAttempts)
        {
        ...
        if (!IsProbablyPrime(p, 20))
            throw new InvalidOperationException("Failed to generate safe prime");
